...
<Tabs>
  <Tab title="iOS">
    ...
    <Steps>
      ...
      Note, this test case will only work if you have Apple News on your device and have opened it at least once before.
      Example test:
        ```swift
        func testAppleNews() throws {
            ...
            q.execute("click the sports tab")
            q.waitUntil("the sports tab has loaded")
            q.execute("scroll down")
            q.execute("tap the first article")
            q.assertCondition("The screen shows a Get Started button")
        }
    ```
      </Step>
  ...
  <Tab title="Android">
    ...
    <Steps>
      ...
      <Step title="Import qaml to your Android Instrumented Tests">
        ...
        Import qaml and create your first test. Note, `myFirstTest` is intended to run on a simulated Pixel 3a starting from the homescreen with the search bar visible and Youtube app installed.
        <CodeGroup>
          ```java Java
